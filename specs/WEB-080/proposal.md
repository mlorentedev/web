...
- [ ] **AC5 — No regression.** `IdpPage.astro` deleted; `/lab` and `/es/lab` build; `IdpStrip.astro` on both landings still builds and reads from `platform.ts`, not from hand-written HTML; **0 axe violations** on `/lab` and `/es/lab` (Playwright + axe-core). The page ships **exactly one script** — the reachability console — and no hydrated island.
