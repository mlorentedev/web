...
```mermaid
flowchart TD
    HS["Headscale Control Plane\nvpn.kubelab.live (162.55.57.175)"]
    HS -.->|"Noise protocol coordination"| mesh

    subgraph mesh["Encrypted WireGuard Mesh (100.64.0.0/24)"]
        direction TB
        subgraph Remote["Remote Nodes"]
            direction TB
            MSI["MSI Workstation · 100.64.0.1"]
            VPS["Hetzner VPS · 100.64.0.2"]
            RPI3["RPi3 · 100.64.0.6"]
            MSI ~~~ VPS ~~~ RPI3
        end

        subgraph LAN["Home LAN (172.16.1.0/24)"]
            direction TB
            RPi4["RPi4 Gateway · 100.64.0.5"]
            K3S["k3s-server · 100.64.0.4"]
            A1["agent-1 · 100.64.0.7"]
            A2["agent-2 · 100.64.0.9"]
            BEE["Beelink · 100.64.0.3"]
            JET["Jetson Nano · 100.64.0.8"]
            RPi4 ~~~ K3S ~~~ A1 ~~~ A2 ~~~ BEE ~~~ JET
        end

        Remote <-->|"direct p2p WireGuard tunnels"| LAN
    end
```
