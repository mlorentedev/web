...
flowchart TD
    subgraph alwayson["Always-on cloud"]
        direction TB
        GCP["gcp1 · GCP e2-small\nArgo CD hub, single node"]
        VPS["Hetzner VPS · ARM64, 7.5GB\nProduction K3s, single node\nTraefik · API · Web · Authelia · Grafana"]
        RPI3["RPi3 · 0.9GB\nUptime Kuma, out-of-band"]
        GCP -->|"reconciles"| VPS
        VPS -.->|"mesh"| RPI3
    end
    ...
    subgraph homelab["Homelab, on demand"]
        direction TB
        RPI4["RPi4 gateway\nPi-hole + CoreDNS"]
        subgraph k3s_nodes["K3s & Workspaces"]
            direction TB
            ACE1["ace1 · N95, 11.5GB\nStaging K3s, single node"]
            ACE2["ace2 · N95, 11.5GB\nDev node, agent workspaces"]
            ACE1 ~~~ ACE2
        end
        subgraph srv_nodes["Services & Edge"]
            direction TB
            BEE["Beelink · N95, 7.5GB\nGitea · CI runner · MinIO"]
            JET["Jetson Nano · 3.9GB\nLocal inference"]
            BEE ~~~ JET
        end
        RPI4 --> ACE1
        RPI4 --> ACE2
